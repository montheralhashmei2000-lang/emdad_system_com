...
function btnState(on){var b=$('btnLogin');if(!b)return;b.disabled=on;b.classList.toggle('is-loading',!!on);b.setAttribute('aria-busy',on?'true':'false');b.textContent=on?'جارٍ التحقق…':'دخول إلى النظام';}
...
/* إصلاح جوهري (فحص v7): كانت هذه الدالة تضع بيانات الملف الشخصي (role, permissions,
   status, approved...) داخل me.state.profile فقط، بينما كل فحوصات الصلاحيات في
   التطبيق (can(), hasPerm() ...) تقرأ me.state.role / me.state.permissions مباشرة
   (بلا .profile). النتيجة: كل مستخدم يسجّل دخوله محليًا — حتى «المدير» نفسه — كان
   يُعامَل كمستخدم بلا أي صلاحيات في كل شاشة تقريبًا (شاشة التفريدة، وغيرها كثير)،
   لأن me.state.role كان دائمًا undefined. الإصلاح: نسخ كل حقول الملف الشخصي مباشرة
   إلى me.state (تمامًا كما كان يحدث في المسار القديم القائم على Firebase). */
function setMeState(profile,email,username){try{var me=window.me;if(me){
  me.state=me.state||{};
  Object.assign(me.state, profile||{});
  me.state.name=(profile&&profile.name)?profile.name:username;
  me.state.email=email;
  me.state.uid=(profile&&profile.id)?profile.id:me.state.uid;
  me.state.username=username;
  me.state.profile=profile||{}; // نُبقيها للتوافق مع أي كود آخر يقرأ .profile
}}catch(e){}}
...
async function afterFirebaseLogin(us){
  try{
    var email=toEmail(us);var uid=(window.auth&&window.auth.currentUser)?window.auth.currentUser.uid:null;
    ...
    if(uid&&window.db){
      try{var d=await window.db.collection('users').doc(uid).get();
        if(d.exists){var dd=d.data();
          if(dd.status!=='active'&&dd.status!==true){if(window.auth)window.auth.signOut().catch(function(){});lErr('✖ الحساب غير مفعّل — اطلب من مدير النظام تفعيل حسابك');return false;}
          if(dd.approved===false){if(window.auth)window.auth.signOut().catch(function(){});lErr('✖ الحساب غير معتمد بعد — انتظر اعتماد المدير');return false;}
...
async function doLogin(){
  ...
  try{
    ...
    /* حسابات «إضافة مستخدم» تُحفظ في مخزن الحسابات بالبريد، ويُربط بها ملف في users
       (الاسم، الدور، الصلاحيات، الأدوار، نطاق المستودعات). نبحث عن الحساب باسم المستخدم
       أو البريد، ثم ندخل ببريده؛ فتُحمِّل ensureProfile الملف كاملًا وتتحقق من التفعيل. */
    var acct=await findUserAccount(u);
    if(acct&&window.auth&&typeof window.auth.createLocalAccount==='function'){
      try{await window.auth.signInWithEmailAndPassword(acct.email,p);lockStore(user,{fails:0,until:0});lMsg('✔ جارٍ تحميل الحساب…');return;}
      catch(e){/* كلمة مرور خاطئة: نكمل إلى عدّاد المحاولات */}
    }
    if(window.IMDAD_ALLOW_FIREBASE_AUTH===true && window.auth && window.db){
      await window.auth.signInWithEmailAndPassword(toEmail(u),p);
      await afterFirebaseLogin(u);
...
async function localLogin(user,p){
  try{
    if(window.AUTH_LOCAL){
      ...
      if(loc){setMeState(loc.profile,user+DOMAIN,user);await mergeUserDoc(user);sesSet({user:user,at:now(),exp:now()+12*3600*1000,profile:loc.profile});nof('🌐 تم الدخول محليًا (بدون إنترنت)');enterApp();return true;}
    }
...
/* البحث عن ملف المستخدم باسم الدخول أو البريد */
async function findUserAccount(input){
  try{
    if(!window.db)return null;var q=norm(input).toLowerCase();if(!q)return null;
    var s=await window.db.collection('users').limit(2000).get();
    var docs=s.docs.map(function(x){var o=x.data()||{};o.id=x.id;return o;});
    var hit=docs.filter(function(x){return String(x.username||'').toLowerCase()===q;})[0]
      ||docs.filter(function(x){return String(x.email||'').toLowerCase()===q;})[0]
      ||(q.indexOf('@')<0?docs.filter(function(x){return String(x.email||'').toLowerCase()===q+DOMAIN;})[0]:null);
    return hit&&hit.email?hit:null;
  }catch(e){return null;}
}
/* دمج صلاحيات الملف (إن وُجد) مع جلسة الدخول المحلي؛ لا يُنزل دور المدير الأول */
async function mergeUserDoc(user){
  try{
    var acct=await findUserAccount(user);if(!acct||!window.me||!window.me.state)return;
    var s=window.me.state,keepAdmin=s.role==='admin';
    ['permissions','roles','warehouseScope','name','active','approved'].forEach(function(k){if(acct[k]!==undefined)s[k]=acct[k];});
    if(!keepAdmin&&acct.role)s.role=acct.role;
    if(!s.id)s.id=acct.id;
  }catch(e){}
}
  ...
  return new Promise(function(resolve,reject){
    (async function(){
      try{
        ...
        var r=regGet();r[user]={salt:salt,h:h,role:(profile&&profile.role)||'user',profile:profile||{name:user,username:user}};regSet(r);
        if(window.AUTH_LOCAL){window.AUTH_LOCAL.register(user,p,profile||{name:user,username:user,role:(profile&&profile.role)||'user'}).catch(function(){});}
        resolve(true);
      }catch(e){reject(e);}
    })();
  });
...
async function doBootstrap(){
  ...
  if(b){b.disabled=true;b.classList.add('is-loading');b.textContent='جارٍ الإنشاء…';}
  try{
  ...
  finally{if(b){b.disabled=false;b.classList.remove('is-loading');b.textContent='إنشاء حساب المدير والدخول';}}
}
...
function showBootstrapIfNeeded(){
  try{
    ...
    var ses=sesGet();var fireUser=window.auth&&window.auth.currentUser;
...
function bind(){
  var b=$('btnLogin');
  if(b&&!b.dataset.imd){
    var nb=b.cloneNode(true);nb.removeAttribute('id');nb.id='btnLogin';nb.dataset.imd='1';
  ...
  var sh=$('lgShowPass');if(sh&&!sh.dataset.imd){sh.dataset.imd='1';sh.onclick=function(){var x=$('lgPass');if(x){var show=x.type==='password';x.type=show?'text':'password';sh.classList.toggle('on',show);sh.setAttribute('aria-pressed',show?'true':'false');sh.title=show?'إخفاء كلمة المرور':'إظهار كلمة المرور';x.focus();}};}
