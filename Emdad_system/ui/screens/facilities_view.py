...
class FacilitiesView(QWidget):
    ...
    def load_metadata(self):
        ...
        ok_u, data_u = api_service.get_units()
        if ok_u:
            self._units = data_u
    ...
    def _load_facilities(self):
        ...
        ok, data = api_service.get_facilities()
        if not ok:
            return
    ...
    def _create_facility(self):
        ...
        ok, resp = api_service.create_facility(payload)
        if ok:
    ...
    def _load_current_subscriptions(self):
        ...
        ok, subs = api_service.get_facility_subscriptions(f_id)
        if ok:
            for s in subs:
                uid = s.get("unit_id")
    ...
    def _save_subscriptions(self):
        ...
        ok, resp = api_service.subscribe_units(payload)
        if ok:
    ...
    def _create_daily_log(self):
        ...
        ok, resp = api_service.create_daily_log(payload)
        if ok:
    ...
    def _load_daily_logs(self):
        ...
        ok, logs = api_service.get_daily_logs(f_id)
        if not ok:
            return
